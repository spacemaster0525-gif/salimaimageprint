...
import com.salimaprint.app.render.ImageUtils
...
class MainActivity : AppCompatActivity() {
    ...
    private fun printImage(
        ...
        val printWidth = when (language) {
            "PCL" -> 640
            else -> 384
        }
        val scaledBitmap = ImageUtils.scaleToPrinterWidth(bitmap, printWidth)

        val payload: ByteArray = when (language) {
            "PCL" -> PclRenderer.render(scaledBitmap)
            else -> EscPosRenderer.render(scaledBitmap)
        }
